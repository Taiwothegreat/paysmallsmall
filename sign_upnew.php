...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if ($stmt->execute()) {
    ...
    try {
        $mail->isSMTP();
        $mail->Host = 'mail.paysmallsmall.org'; // Bluehost mail server
        $mail->SMTPAuth = true;
        $mail->Username = 'info@paysmallsmall.org'; // your domain email
        $mail->Password = 'czX^%9XZFtJR#@'; // actual password
        $mail->SMTPSecure = 'ssl';
        $mail->Port = 465;
...
$mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;
        ...
        <body style='font-family: Arial, sans-serif; background-color: #f9f9f9; padding: 20px;'>
          <div style='max-width: 600px; margin: auto; background: #ffffff; border-radius: 10px; padding: 30px; box-shadow: 0 0 10px rgba(0,0,0,0.1);'>
            ...
            <p><a href='https://www.paysmallsmall.org/product.html' style='display:inline-block; padding:10px 20px; background-color:#2c3e50; color:#ffffff; text-decoration:none; border-radius:5px;'>Explore Products</a></p>
